...
namespace threads {
    namespace policies {
        ...
        private:
            ...
            static bool work_items_ordering(
                thread_description* left, thread_description* right)
            {
                return left->get_deadline() > right->get_deadline();
            };

            typedef std::priority_queue<thread_description*,
        ...
        protected:
                ...
                std::unique_lock<mutex_type>& lk, bool steal = false)
            {
                ...
                while (add_count-- && addfrom->new_tasks_.pop(task, steal))
                {
                    ...
                    util::block_profiler_wrapper<add_new_tag> bp(
                        add_new_logger_);
                    ...
                    HPX_ASSERT(
                        thread_map_.find(thrd.get()) != thread_map_.end());
                ...
                std::unique_lock<mutex_type>& lk, bool steal = false)
            {
                ...
                if (HPX_LIKELY(max_count_))
                {
                    ...
                    {    //-V104
                        ...
                        add_count =
                            static_cast<std::int64_t>(max_count_ - count);
                    ...
                    {
                        add_count =
                            min_add_new_count;    // add this number of threads
                        max_count_ +=
                            min_add_new_count;    // increase max_count //-V101
                    }
                    else
                    {
                        return false;
            ...
            bool cleanup_terminated_locked_helper(bool delete_all = false)
            {
                ...
                if (delete_all)
                {
                    ...
                    while (terminated_items_.pop(todelete))
                    {
                        ...
                        HPX_ASSERT(
                            thread_map_.find(todelete) != thread_map_.end());
                ...
                else
                {
                    ...
                    std::int64_t delete_count = (std::max)(
        ...
        public:
            bool cleanup_terminated(bool delete_all = false)
            {
                ...
                if (delete_all)
                {
                    ...
                    while (true)
                    {
                        ...
                        {
                            thread_map_is_empty = (thread_map_count_ == 0) &&
                                (new_tasks_count_ == 0);
                            break;
                        }
                    }
            ...
            edf_queue(std::size_t queue_num = std::size_t(-1),
              ...
              : min_tasks_to_steal_pending(
                    detail::get_min_tasks_to_steal_pending())
              , min_tasks_to_steal_staged(
                    detail::get_min_tasks_to_steal_staged())
            ...
            {
                max_count_ =
                    (0 == max_count) ? max_thread_count : max_count;    //-V105
            }
                ...
                bool allow_stealing = false, bool steal = false) HPX_HOT
            {
                ...
                if (allow_stealing &&
                    min_tasks_to_steal_pending > work_items_count)
                {
                    return false;
                }
                ...
                threads::thread_data* thrd, std::int64_t& busy_count)
            {
                if (thrd->get_pool() == &memory_pool_)
                {
                    ...
                    {
                        cleanup_terminated(
                            true);    // clean up all terminated threads
                    }
                    return true;
                }
                return false;
                ...
                thread_state_enum state = unknown) const
            {
                ...
                if (state == unknown)
                {
                    std::lock_guard<mutex_type> lk(mtx_);
                    thread_map_type::const_iterator end = thread_map_.end();
                    for (thread_map_type::const_iterator it =
                             thread_map_.begin();
                         it != end;
                         ++it)
                    {
        ...
        private:
            ...
            threads::thread_pool
                memory_pool_;    ///< OS thread local memory pools for
                                 ///< HPX-threads
