...
void
work_for(std::chrono::steady_clock::duration duration)
{
...
void
work_for_noyield(std::chrono::steady_clock::duration duration)
{
    std::this_thread::sleep_for(duration);
}

void
work_for_print(std::chrono::steady_clock::duration duration)
{
...
yield_test()
{
    hpx::async([]() {
        while (true) {
            ...
            auto duration =
            std::chrono::duration_cast<std::chrono::microseconds>(t2 - t1).count();
            std::cout << "yielding took " << duration << " microseconds" << std::endl;
        }
    })
    .get();
}
...
multiply_matrices(std::vector<std::vector<int>> a,
                           ...
                           std::vector<std::vector<int>> b)
{
    ...
    for (size_t i = 0; i < a_rows; i++) {
        futures[i] = hpx::async(executor2, [a_cols, b_cols, i, &a, &b, &result]() {
            result[i] = std::vector<int>(b_cols);
            for (size_t j = 0; j < b_cols; j++) {
                auto v = 0;
                for (size_t k = 0; k < a_cols; k++) {
...
void
print_matrix(std::vector<std::vector<int>> m)
{
...
{
public:
    task(size_t task_id, std::chrono::steady_clock::duration activation_interval,
    ...
    {
        task_timings_.reserve(4 *
                              (std::chrono::seconds(10) / activation_interval_));
    }
...
private:
    bool do_work()
    {
        ...
        if (last_finished_activation_ + 1 < activation) {
            std::cout << "Task " << task_id_ << " is overwhelmed. (last finished "
...
tasks_tests()
{
    ...
    for (size_t i = 0; i < tasks.size(); i++) {
        ...
        for (size_t j = 0; j < tasks[i].task_timings_.size(); j++) {
            ...
            out << "task " << i << " activation " << timing.activation << " " << name
            << " " << timing.time.time_since_epoch().count() << std::endl;
        }
    }
    out.close();
...
test_pools()
{
    ...
    {
        hpx::async(executor1, []() {
            hpx::cout << "executor1 running on scheduler " << hpx::this_thread::get_id().native_handle().get()->get_scheduler_base() << hpx::endl;
        }).get();
        hpx::async(executor2, []() {
            hpx::cout << "executor2 running on scheduler " << hpx::this_thread::get_id().native_handle().get()->get_scheduler_base() << hpx::endl;
        }).get();
    }
}
...
int
hpx_main(boost::program_options::variables_map& vm)
{
    test_pools();
    no_work_task_overhead();
    work_for_overhead();
...
main(int argc, char* argv[])
{
    ...
    rp.create_thread_pool(
                          ...
                          -> std::unique_ptr<hpx::threads::detail::thread_pool_base> {
                              ...
                              auto thread_pool = new hpx::threads::detail::scheduled_thread_pool<edf_sched>(
                                                                                                                      std::move(scheduler), notifier, pool_index, pool_name, mode,
                                                                                                                      thread_offset);
                              
                              std::unique_ptr<hpx::threads::detail::thread_pool_base> pool(thread_pool);
                              return pool;
                          });
