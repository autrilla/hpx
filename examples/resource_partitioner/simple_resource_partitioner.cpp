...
public:
    ...
    task(size_t task_id,
        std::chrono::steady_clock::duration activation_interval,
        size_t priority, std::chrono::steady_clock::duration work_duration)
      : task_timings_()
      , task_id_(task_id)
      , activation_interval_(activation_interval)
      , work_duration_(work_duration)
...
private:
    bool do_work()
    {
        ...
        if (priority != 1)
        {
            deadline += std::chrono::milliseconds(priority);
        }
        else
        {
            deadline = now + relative_deadline_;
        }
...
void tasks_tests()
{
    ...
    std::ifstream priority_infile("tasks_priority.txt");
    size_t priority;
    while (priority_infile >> period >> work_time >> priority)
    {
        tasks.push_back(task(task_id++, std::chrono::microseconds(period),
            priority, std::chrono::microseconds(work_time)));
    }
    ...
    for (size_t i = 0; i < tasks.size(); i++)
    {
        out << "task_details " << i << " period "
            ...
            << tasks[i].relative_deadline_.count() << " priority "
            << tasks[i].priority << " work_duration "
